...
function updateProduto($conn, $id, $nome_produto, $quantidade_min, $quantidade_atual) {
    $sql = "UPDATE estoque SET nome_produto = :nome_produto, quantidade_min = :quantidade_min, quantidade_atual = :quantidade_atual WHERE id = :id"; //mudar
    $stmt = $conn->prepare($sql);
    $stmt->bindParam(':id', $id);
    $stmt->bindParam(':nome_produto', $nome_produto);
    $stmt->bindParam(':quantidade_min', $quantidade_min);
    $stmt->bindParam(':quantiade_minima', $quantidade_atual);
    $stmt->execute();
}
